...
void Logger::SetLogPreferences(LogLevel level = LogLevel::ERROR) {
	...
	if (!logFile.good()) {
    	...
    	// create a local log file for SFT module instead
		std::string fwd_slash = "/";
    	std::string logFileName = "sft_log.txt";
		...
		const char *cstr = mkdir_cmd.c_str();
   		status = system(cstr);
   		if (status == -1)
   		   std::cerr << "Error(" << (errno) << ") creating log file directory for SFT: " << logFileDir << std::endl;
   		else
   		   std::cout << "Log directory for SFT: " << logFileDir <<std::endl;
		// create a local log file for SFT module
		logFilePath = logFileDir+logFileName;
		logFile.open(logFilePath, ios::out | ios::app);
		if (!logFile.good()) {
			std::cerr << "Can't Open local directory Log File for SFT:" << logFilePath <<std::endl;			
		}
		else {
			std::cout << "Logging SFT module messages instead into: " << logFilePath << std::endl;
		}
			
	}
	else {
		std::cout << "Log File Path for SFT:" << logFilePath << std::endl;
	}
...
void Logger::Log(std::string message, LogLevel messageLevel = LogLevel::DEBUG) {
	LoggingModule module=LoggingModule::SFT;
...
void Logger::setup_logger(void) {
	//return if this method has already been called
	if (loggerInstance != nullptr) return;
